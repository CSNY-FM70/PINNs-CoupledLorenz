...
OptErr = [120000, 5e-4, [1, 1, 1, 1, 1, 1], [None]*6, [None]*6]
...
for i in range(-2, 0):
    initial_boundary.append(pow(10, i))

for i in range(0, 2):
    data.append(pow(10, i))

savePath_Data = "OptError_Data_Plots/OptError_InitialBoundaryXData" + \
    "_"+str(initial_boundary[0])+"X"+str(initial_boundary[-1])+".txt"
L_i = np.zeros(len(initializer))
means = np.zeros((len(initial_boundary), len(data)))
standard_var = np.zeros((len(initial_boundary), len(data)))
...
with open(savePath_Data, "w") as file:
    for i in range(len(initial_boundary)):
        for e in range(len(data)):
            ...
            content = [str(initial_boundary[i]), ",", str(data[e]), ",", str(
