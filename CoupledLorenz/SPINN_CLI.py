...
def main(Physics, ApproxErr, SampErr, OptErr, 
    ...
    def feature_transform1(t):
        #t = char_t * t
        return tf.concat(
            (t, tf.sin(np.pi*t), tf.sin(2*np.pi * t), tf.sin(3.*np.pi* t),
            tf.sin(4.*np.pi*t), tf.sin(5*np.pi * t)),
            axis=1,
        )

    def feature_transform2(t):
        t = char_t * t
        return t

    net.apply_feature_transform(feature_transform2)
    def output_transform1(t, y):
        idx = observe_t.shape[0] -1
        k = (ob_y[idx] - ob_y[0]) / (observe_t[idx] - observe_t[0])
        b = (observe_t[idx] * ob_y[0] - observe_t[0] * ob_y[idx]) / (
            observe_t[idx] - observe_t[0]
        )
        linear = k * t + b
        factor = tf.math.tanh(t) * tf.math.tanh(idx - t)
        return linear + factor * tf.constant([1e1, 1e1, 1e1, 1, 1, 1]) * y
    
    def output_transform2(t, y):
       
        return tf.constant([1e1, 1e1, 1e1, 1, 1, 1]) * y
